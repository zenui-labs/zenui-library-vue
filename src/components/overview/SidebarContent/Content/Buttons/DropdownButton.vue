...
import {
  ActionButtonPreviewCode,
  PublishButtonPreviewCode,
  SendButtonWithArrowPreviewCode,
} from "./PreviewCodes/DropdownButtonPreviewCodes";
...
<template>
  ...
  >
    <div>
      ...
      <ToggleTab
        :code="publishCode"
        @update:code="(val) => (publishCode = val)"
      ...
      <ComponentWrapper>
        <div
          ...
          :class="[
            'p-8',
            publishButtonActive ? 'mb-20' : 'mb-4',
            'flex flex-wrap items-center gap-5 justify-center transition-all duration-300',
          ]"
        >
          <div
          ...
          >
            <button
              class="text-[1rem] px-6 py-1.5 transition-all duration-500 cursor-auto"
            >
              {{ publishButtonText }}
            </button>
            <div
              @click="publishButtonActive = !publishButtonActive"
            ...
            >
              <li
                v-for="(item, index) in publishButtonContent"
                :key="index"
      ...
      <ToggleTab
        ...
        @update:code="(val) => (actionButtonCode = val)"
        :preview="actionButtonPreview"
        @update:preview="(val) => (actionButtonPreview = val)"
      />
      <ComponentWrapper>
        <div
          v-if="actionButtonPreview"
          :class="[
            'p-8',
            actionButtonActive ? 'mb-28' : 'mb-4',
            'flex flex-wrap items-center gap-5 justify-center transition-all duration-300',
          ]"
        >
          <div
          ...
          >
            ...
            <div
              @click="actionButtonActive = !actionButtonActive"
              class="bg-brandColor w-[50px] py-1.5 flex items-center justify-center cursor-pointer rounded-r publishButton"
            >
              <Icon icon="mdi:menu-down" class="text-[2rem]" />
            </div>
            ...
            >
              <li
                v-for="(item, index) in actionContents"
                :key="index"
      ...
      />
      <ToggleTab
        :code="arrowSendButtonCode"
        @update:code="(val) => (arrowSendButtonCode = val)"
        :preview="arrowSendButtonPreview"
        @update:preview="(val) => (arrowSendButtonPreview = val)"
      />
      <ComponentWrapper>
        <div
          v-if="arrowSendButtonPreview"
          :class="[
            'p-8',
            actionButtonActive2 ? 'mb-32' : 'mb-4',
            'flex flex-wrap items-center gap-5 justify-center transition-all duration-300',
          ]"
        >
          <div
          ...
          >
            ...
            <div
              @click="actionButtonActive2 = !actionButtonActive2"
              class="bg-brandColor w-[50px] py-1.5 flex items-center justify-center cursor-pointer rounded-r publishButton"
            >
              <Icon icon="mdi:menu-down" class="text-[2rem]" />
            </div>
            <ul
              :class="
                actionButtonActive2
                  ? 'opacity-100 z-20 translate-y-4'
            ...
            >
              <div
                class="absolute -top-[8px] dark:bg-slate-800 dark:border-slate-700 right-3 border-l border-b border-[#e6e6e6] bg-white w-[15px] h-[15px] rotate-[135deg]"
              ></div>
              <li
                v-for="(item, index) in sendButtonContent"
                :key="index"
        ...
        <ShowCode
          v-if="arrowSendButtonCode"
          :code="SendButtonWithArrowPreviewCode"
        />
      </ComponentWrapper>

      <OverviewFooter
        backUrl="/components/auth-buttons"
        backName="auth button"
        forwardUrl="/components/drag-and-drop"
        forwardName="drag & drop"
      />
    </div>

    <ContentNavbar
      :contents="dropdownButtonContents"
