...
<template>
  <aside class="flex items-start justify-between gap-6 w-full 640px:pl-[2.5rem] px-6 640px:px-10">
    <div>
      ...
      <ToggleTab :code="publishCode" @update:code="(val)=>(publishCode = val)" :preview="publishPreview"
                 @update:preview="(val)=> publishPreview = val"/>
      <ComponentWrapper>
        <div v-if="publishPreview"
             :class="['p-8', publishButtonActive ? 'mb-20' : 'mb-4', 'flex flex-wrap items-center gap-5 justify-center transition-all duration-300']">
          <div
              class="flex items-center rounded bg-brandColor/80 border-none outline-none text-secondary justify-between relative">
            <button class="text-[1rem] px-6 py-1.5 transition-all duration-500 cursor-auto">{{
                publishButtonText
              }}
            </button>
            <div @click="publishButtonActive = !publishButtonActive"
                 class="bg-brandColor w-[50px] py-1.5 flex items-center justify-center cursor-pointer rounded-r publishButton">
            ...
            <ul :class="publishButtonActive ? 'opacity-100 z-20 translate-y-0' : 'opacity-0 z-[-1] translate-y-[-5px]'"
              ...
              <li v-for="(item, index) in publishButtonContent" :key="index"
                  class="py-2 px-6 hover:bg-gray-50 dark:hover:bg-slate-900/40 rounded cursor-pointer"
                  @click="handlePublishButtonClick(item)">
                {{ item }}
              </li>
            </ul>
          </div>
      ...
      <ToggleTab :code="actionButtonCode" @update:code="(val)=>actionButtonCode=val" :preview="actionButtonPreview"
                 @update:preview="(val)=>actionButtonPreview = val"/>
      <ComponentWrapper>
        <div v-if="actionButtonPreview"
             :class="['p-8', actionButtonActive ? 'mb-28' : 'mb-4', 'flex flex-wrap items-center gap-5 justify-center transition-all duration-300']">
          <div
              class="flex items-center rounded bg-brandColor/80 border-none outline-none text-secondary justify-between relative">
            <button class="text-[1rem] px-6 py-1.5 transition-all duration-500 cursor-auto">{{
                actionButtonText
            ...
            <div @click="actionButtonActive = !actionButtonActive"
                 class="bg-brandColor w-[50px] py-1.5 flex items-center justify-center cursor-pointer rounded-r publishButton">
              <Icon icon="mdi:menu-down" class="text-[2rem]"/>
            </div>
            <ul :class="actionButtonActive ? 'opacity-100 z-20 translate-y-0' : 'opacity-0 z-[-1] translate-y-[-5px]'"
              ...
              <li v-for="(item, index) in actionContents" :key="index"
                  class="py-2 px-3 flex items-center dark:hover:bg-slate-900/40 gap-[5px] hover:bg-gray-50 rounded cursor-pointer"
                  @click="handleActionButtonClick(item.label)">
                <Icon :icon="item.icon" class="text-brandColor"/>
                {{ item.label }}
              </li>
            </ul>
          </div>
      ...
      <ToggleTab :code="arrowSendButtonCode" @update:code="(val)=>(arrowSendButtonCode = val)"
                 :preview="arrowSendButtonPreview" @update:preview="(val)=>(arrowSendButtonPreview=val)"/>
      <ComponentWrapper>
        <div v-if="arrowSendButtonPreview"
             :class="['p-8', actionButtonActive2 ? 'mb-32' : 'mb-4', 'flex flex-wrap items-center gap-5 justify-center transition-all duration-300']">
          <div
              class="flex items-center rounded bg-brandColor/80 border-none outline-none text-secondary justify-between relative">
            <button class="text-[1rem] px-6 py-1.5 transition-all duration-500 cursor-auto">{{
                sendButtonText
            ...
            <div @click="actionButtonActive2 = !actionButtonActive2"
                 class="bg-brandColor w-[50px] py-1.5 flex items-center justify-center cursor-pointer rounded-r publishButton">
              <Icon icon="mdi:menu-down" class="text-[2rem]"/>
            </div>
            <ul :class="actionButtonActive2 ? 'opacity-100 z-20 translate-y-4' : 'opacity-0 z-[-1] translate-y-[-20px]'"
              ...
              <div
                  class="absolute -top-[8px] dark:bg-slate-800 dark:border-slate-700 right-3 border-l border-b border-[#e6e6e6] bg-white w-[15px] h-[15px] rotate-[135deg]"></div>
              <li v-for="(item, index) in sendButtonContent" :key="index"
                  class="z-20 py-2 px-3 dark:hover:bg-slate-900/40 flex items-center gap-[8px] hover:bg-gray-50 rounded cursor-pointer"
                  @click="handleSendButtonClick(item.label)">
                <Icon :icon="item.icon" class="text-brandColor"/>
                {{ item.label }}
              </li>
            </ul>
      ...
      <OverviewFooter backUrl="/components/auth-buttons" backName="auth button" forwardUrl="/components/drag-and-drop"
                      forwardName="drag & drop"/>
    </div>
