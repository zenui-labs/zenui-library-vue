...
import {numberInputContents} from "@/Utils/ContentsConfig/InputContents.js";
import {useScrollSpy} from "@/CustomHooks/useScrollSpy.js";
import {ref} from "vue";
import {Icon} from "@iconify/vue";
import {
  miniNumberInputCodes,
  roundedNumberInputCodes,
  roundedpositionNumberInputCodes
} from "@/components/overview/SidebarContent/Content/Inputs/PreviewCodes/NumberInputPreviewCodes.js";

const miniInputValue = ref(0);
...
const handleMiniInputValueChange = (e) => {
  miniInputValue.value = e.target.value;
};

const handleRoundedIncrement = () => {
  roundedInputValue.value = roundedInputValue.value + 1;
};
...
const handleRoundedInputValueChange = (e) => {
  roundedInputValue.value = e.target.value;
};
</script>
  ...
  >
    <div>
      ...
      <ComponentDescription
          text="This is an input text field. Use it to enter your information, which
            will be processed accordingly."
      />

      <ToggleTab
          :preview="miniNumberInputPreview"
          @update:preview="(val) => (miniNumberInputPreview = val)"
          @update:code="(val) => (miniNumberInputCode = val)"
          :code="miniNumberInputCode"
      />

      <ComponentWrapper>
        <div
            v-if="miniNumberInputPreview"
      ...
      <ComponentDescription
          text="This is a bottom-bordered input text field. Enter your information
            here for it to be processed accordingly."
      />

      <ToggleTab
          :preview="roundedButtonPreview"
          @update:preview="(val) => (roundedButtonPreview = val)"
          @update:code="(val) => (roundedButtonCode = val)"
          :code="roundedButtonCode"
      />

      <ComponentWrapper>
        <div
            v-if="roundedButtonPreview"
            class="p-8 mb-4 flex items-center flex-col gap-5 justify-center"
        >
          <div
          ...
          >
            ...
            <input
                ...
                @input="handleRoundedInputValueChange"
            />
            <button
                class="bg-gray-100 p-[10px] dark:bg-slate-800 dark:text-[#abc2d3] rounded-full text-gray-700 text-[1.1rem]"
                @click="handleRoundedIncrement"
            >
              <Icon icon="majesticons:plus-line"/>
            </button>
          </div>
        </div>
        <ShowCode v-else :code="roundedNumberInputCodes"/>
      </ComponentWrapper>
      ...
      <ToggleTab
          :preview="roundedButtonPositionPreview"
          @update:preview="(val) => (roundedButtonPositionPreview = val)"
          @update:code="(val) => (roundedButtonPositionCode = val)"
          :code="roundedButtonPositionCode"
      />

      <ComponentWrapper>
        <div
            v-if="roundedButtonPositionPreview"
            class="p-8 mb-4 flex items-center flex-col gap-5 justify-center"
        >
          <div
              class="flex px-2 py-0.5 items-center dark:border-slate-700 mx-auto border border-gray-200 rounded-md"
        ...
        <ShowCode v-else :code="roundedpositionNumberInputCodes"/>
      </ComponentWrapper>

      <OverviewFooter
          backName="Textarea Input"
          backUrl="/components/input-textarea"
          forwardName="Checkbox Input"
          forwardUrl="/components/input-checkbox"
      />
    </div>

    <ContentNavbar
        :contents="numberInputContents"
